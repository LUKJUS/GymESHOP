...
public class KlientasController : Controller
{
	...
	[HttpPost]
	public ActionResult Create(KlientasCE client)
	{
		//do not allow creation of entity with 'asmensKodas' field matching existing one
		var match = KlientasRepo.Find(client.Klientas.Id);
	...
	[HttpPost]
	public ActionResult Edit(int id, KlientasCE client)
	{
		//form field validation passed?
		if (ModelState.IsValid)
		{
			KlientasRepo.Update(client);
